...
public class RfbProto {
	...
	public static final int SecTypeUltra34 = 0xfffffffa;
	...
	public final static int SetPixelFormat = 0, FixColourMapEntries = 1, SetEncodings = 2, FramebufferUpdateRequest = 3,
			KeyboardEvent = 4, PointerEvent = 5, ClientCutText = 6;

	// Supported encodings and pseudo-encodings
	public final static int EncodingRaw = 0, EncodingCopyRect = 1, EncodingRRE = 2, EncodingCoRRE = 4, EncodingHextile = 5,
	...
	public final static String SigEncodingRaw = "RAW_____", SigEncodingCopyRect = "COPYRECT", SigEncodingRRE = "RRE_____",
	...
	public final static int HextileRaw = 1, HextileBackgroundSpecified = 2, HextileForegroundSpecified = 4,
	...
	public final static int TightExplicitFilter = 0x04, TightFill = 0x08, TightJpeg = 0x09, TightMaxSubencoding = 0x09,
			TightFilterCopy = 0x00, TightFilterPalette = 0x01, TightFilterGradient = 0x02;

	// Constants used for UltraVNC chat extension
	public final static int CHAT_OPEN = -1, CHAT_CLOSE = -2, CHAT_FINISHED = -3;
	...
	public RfbProto(String h, int p)  {
		// - viewer = v;
		host = h;
		port = p;
	}
	...
	public int fbWidth() { return framebufferWidth; }
	public int fbHeight() { return framebufferHeight; }
	...
	*/
	...
	public void readFramebufferUpdate() throws IOException {
		is.readByte();
		updateNRects = (short)(is.readUnsignedShort() & 0xFFFF);
	...
	public void readFramebufferUpdateRectHdr() throws Exception {
		updateRectX = (short)(is.readUnsignedShort() & 0xFFFF);
		updateRectY = (short)(is.readUnsignedShort() & 0xFFFF);
		updateRectW = (short)(is.readUnsignedShort() & 0xFFFF);
		updateRectH = (short)(is.readUnsignedShort() & 0xFFFF);
	...
	public synchronized void writeSetPixelFormat(int bitsPerPixel, int depth, boolean bigEndian, boolean trueColour,
			int redMax, int greenMax, int blueMax, int redShift, int greenShift, int blueShift, boolean fGreyScale) // sf@2005)
			throws IOException {
		byte[] b = new byte[20];
	...
	synchronized void writeFixColourMapEntries(int firstColour, int nColours, int[] red, int[] green, int[] blue)
			throws IOException {
	...
	/**
	 * Write a pointer event message. We may need to send modifier key events around
	 * it to set the correct modifier state.
	 * 
	 * @param x
	 * @param y
	 * @param modifiers
	...
	void writeCtrlAltDel() throws IOException {
		...
		final int CTRLALT = KeyEvent.META_CTRL_ON | KeyEvent.META_ALT_ON;
		try {
			// Press
			eventBufLen = 0;
	...
	void writeModifierKeyEvents(int newModifiers) {
		if ((newModifiers & KeyEvent.META_CTRL_ON) != (oldModifiers & KeyEvent.META_CTRL_ON))
			writeKeyEvent(0xffe3, (newModifiers & KeyEvent.META_CTRL_ON) != 0);

		if ((newModifiers & KeyEvent.META_SHIFT_ON) != (oldModifiers & KeyEvent.META_SHIFT_ON))
			writeKeyEvent(0xffe1, (newModifiers & KeyEvent.META_SHIFT_ON) != 0);

		if ((newModifiers & KeyEvent.META_SYM_ON) != (oldModifiers & KeyEvent.META_SYM_ON))
			writeKeyEvent(0xffe7, (newModifiers & KeyEvent.META_SYM_ON) != 0);

		if ((newModifiers & KeyEvent.META_ALT_ON) != (oldModifiers & KeyEvent.META_ALT_ON))
			writeKeyEvent(0xffe9, (newModifiers & KeyEvent.META_ALT_ON) != 0);
